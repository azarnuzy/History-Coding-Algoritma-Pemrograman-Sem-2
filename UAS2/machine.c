...
int CHECKSUBKATA(char kata[]) {
    if(strcmp(kata, "10001") == 0){      //kondisi untuk mengecek jika vokal konsonan konsonan konsonan vokal maka nilai dari kata = "10001"
        return 1;
    }
...
void CARIVOKAL(char pita[]) {
    ...
    int leng = len - 5 + 1;     //sebagai panjang dari perulangan yang akan dilakukan 
    for(i=0; i<leng; i++) {     //perulangan untuk mencari subkata sesuai dengan spesifikasi 
        ...
        if(CHECKSUBKATA(GETSUBKATA()) == 1){                //kondisi jika memenuhi spesifikasi
            strcpy(subsKata[indeksSubKata], GETCKATA());    //menyalin kata ke subskata
            indeksSubKata++;                                //sebagai iterator untuk bertambahnya indeks pada subskata
            i += leng;                                      //bertambah sebanyak huruf ke depan agar perulangan berhenti
        }
    }
}
